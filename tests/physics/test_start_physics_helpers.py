...
def test_save_and_clear_callbacks_returns_saved():
    ...
    saved = ball._save_and_clear_callbacks(id(ball))
...
def test_save_and_clear_callbacks_clears_manager():
    ...
    ball._save_and_clear_callbacks(id(ball))
...
def test_reregister_dependent_callbacks_refreshes_other_sprite():
    """_reregister_dependent_callbacks should refresh callbacks from dependents."""
    ball = play.new_circle(x=0, y=0, radius=20)
    ball.start_physics(obeys_gravity=False)
    ...
    # Simulate what start_physics does: recreate target's physics then reregister
    target.physics._remove()
    from play.physics import Physics as _Physics

    target.physics = _Physics(
        target,
        can_move=False,
        stable=False,
        x_speed=0,
